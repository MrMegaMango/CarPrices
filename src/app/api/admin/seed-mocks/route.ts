...
async function findOrCreateMake(name: string): Promise<{ id: string; name: string }> {
  ...
  const inserted = await sql`
    insert into car_makes (id, name, "createdAt", "updatedAt")
    values (${newId}, ${name}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    returning id, name
  ` as Array<{ id: string; name: string }>
  return inserted[0]
}
...
async function findOrCreateModel(makeId: string, name: string): Promise<{ id: string; name: string }> {
  ...
  const inserted = await sql`
    ...
    values (${newId}, ${name}, ${makeId}, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
    returning id, name
  ` as Array<{ id: string; name: string }>
  return inserted[0]
}
